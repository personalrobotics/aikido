...
public:
  ...
  void evalGradient(const Eigen::VectorXd& _qd,
...
MoveHandStraightVectorField::MoveHandStraightVectorField(
  ...
  , bodynode_(bodynode)
  , start_pose_(bodynode->getTransform())
{
  ...
  double t, Eigen::VectorXd *qd)
{
  ...
  Vector3d const rotation_error = logMap(
  ...
  desired_twist.tail<3>() = linear_feedforward + linear_gain_ * linear_orthogonal_error;

  // TODO: This should be a parameter.
  const double padding = 1e-3;

  // Use LBFGS to find joint angles that won't violate the joint limits.
  const Jacobian jacobian = stateSpace->getMetaSkeleton()->getWorldJacobian(bodynode_);
  ...
  for (size_t i = 0; i < numDofs; ++i)
  {
    ...
    const double positionLowerLimit = stateSpace->getMetaSkeleton()->getPositionLowerLimit(i);
    const double positionUpperLimit = stateSpace->getMetaSkeleton()->getPositionUpperLimit(i);
    const double velocityLowerLimit = stateSpace->getMetaSkeleton()->getVelocityLowerLimit(i);
    const double velocityUpperLimit = stateSpace->getMetaSkeleton()->getVelocityUpperLimit(i);
