...
///
std::tuple<Eigen::VectorXd, Eigen::VectorXd> MyInformedSampler::getStateLimits()
    const
{
  // Get the bounds from the vector
  const auto space
      = si_->getStateSpace()->as<ompl::base::RealVectorStateSpace>();
...
Eigen::VectorXd MyInformedSampler::getGradient(
...
Eigen::VectorXd MyInformedSampler::getInvJacobian(
    const Eigen::VectorXd& curr_state) const
{
  // Assert that the matrix is not empty
  assert(curr_state.size() != 0 or curr_state.size() != 0);
...
double MyInformedSampler::getCost(const Eigen::VectorXd& curr_state) const
{
  ...
  const ompl::base::State* goal_state
      = problem_->getGoal()->as<ompl::base::GoalState>()->getState();

  get_ompl_state(curr_state, tmpState_);

  const ompl::base::OptimizationObjectivePtr optim_obj
      = problem_->getOptimizationObjective();
...
bool MyInformedSampler::isInLevelSet(
    const Eigen::VectorXd& curr_state,
    ...
    double& stateCost) const
{
  ...
  const ::ompl::base::OptimizationObjectivePtr optim_obj
      = problem_->getOptimizationObjective();
  const ::ompl::base::DimtObjective* dimt_obj
      = dynamic_cast<::ompl::base::DimtObjective*>(optim_obj.get());
  ...
  double costToCome
      = dimt_obj
            ->getCostIfSmallerThan(start_state, tmpState_, Cost(thresholdCost))
            .value();
  ...
  double costToGo
      = dimt_obj
            ->getCostIfSmallerThan(
                tmpState_, goal_state, Cost(thresholdCost - costToCome))
            .value();
...
{
  /*
  if (started_ == false)  // Set the timer when the function is called for the
  first time
  {
      started_ = true;
      // set time
  ...
  if (!same_cost(maxCost.value(), prevCost_) or sampleIndex_ >=
  sampleBatchSize_)
  {
  ...
  double *val = static_cast<ompl::base::RealVectorStateSpace::StateType
  *>(statePtr)->values;
...
/// @return true if a sample is gotten false, if not
///
bool MyInformedSampler::sampleUniform(
    State* statePtr, const Cost& minCost, const Cost& maxCost)
{
...
///
Eigen::VectorXd MyInformedSampler::newtonRaphson(
    const Eigen::VectorXd& start, double levelSet_)
{
...
#ifdef USE_NLOPT
double MyInformedSampler::inequalConstraint(
...
Eigen::VectorXd MyInformedSampler::findSolutionInLevelSet(
    Eigen::VectorXd& init, double levelSet_)
{
...
Eigen::MatrixXd MyInformedSampler::sample(
    ...
    std::chrono::high_resolution_clock::duration& duration)
{
  ...
  while (numAcceptedSamples_ < numSamples
         && timeElapsedDouble < batchTimelimit_)
  {
...
{
  acceptanceRatio_
      = static_cast<double>(numAcceptedSamples_)
