...
inline double sigmoid(
    const double& x, const double& a = 200, const double& c = 0)
{
...
///
bool MonteCarloSampler::anyDimensionInViolation(
...
double MonteCarloSampler::getEnergy(const Eigen::VectorXd& curr_state) const
{
  ...
  for (int i = 0; i < curr_state.size(); i++)
  {
    E_region
        += 100
           * sigmoid(std::get<0>(limits)(i) - curr_state(i)); // Lower Limits
    E_region
        += 100
           * sigmoid(curr_state(i) - std::get<1>(limits)(i)); // Higher Limits
  }
...
Eigen::VectorXd MonteCarloSampler::getGradient(
...
//
Eigen::VectorXd MonteCarloSampler::sampleNormal(
...
bool HMCSampler::sampleInLevelSet(Eigen::VectorXd& sample)
{
  ...
  do
  {
    ...
    */
    ...
    double alpha
        = std::min(1.0, std::exp(U_last - U_proposed + K_last - K_proposed));
