...
namespace ompl {
namespace base {
Eigen::VectorXd GibbsSampler::getRandomSample(
...
bool GibbsSampler::sampleInLevelSet(Eigen::VectorXd& sample)
{
  ...
  do
  {
    ...
    q = getRandomSample(
        stateMin_[(numAcceptedSamples_ + skip) % getSpaceDimension()],
...
bool HitAndRunSampler::sampleInLevelSet(Eigen::VectorXd& sample)
{
  ...
  do
  {
    ...
    if (lamda_lower_bound < 0 && lamda_upper_bound > 0)
    {
      ...
      lamda = rand_no1 < 0.5 ? lamda_lower_bound * pow(rand_no2, 1 / power)
                             : lamda_upper_bound * pow(rand_no2, 1 / power);
    }
    else
    {
      ...
      if (lamda_lower_bound > 0)
        lamda = pow(
            pow(lamda_lower_bound, power)
                + rand_no * (pow(lamda_upper_bound, power)
                             - pow(lamda_lower_bound, power)),
            1 / power);
      else if (lamda_upper_bound < 0)
        lamda = -pow(
            pow(lamda_upper_bound, power)
