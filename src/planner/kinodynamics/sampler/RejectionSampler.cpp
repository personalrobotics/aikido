...
bool RejectionSampler::sampleInLevelSet(Eigen::VectorXd& sample)
{
  Eigen::VectorXd newSample
      = getRandomSample(stateMin_, stateMax_, getSpaceDimension());
  ...
  static int cnt = 0;
  if (cnt++%100000 == 0)
      std::cout << "ACCEPTED  " << numAcceptedSamples_ << " REJECTED " <<
  numRejectedSamples_ << std::endl;
  */
...
Eigen::VectorXd RejectionSampler::getRandomSample(
...
///
std::tuple<double, double> HierarchicalRejectionSampler::HRS(
    ...
    double timeLeft)
{
  ...
  else
  {
    ...
    while (Cost(cStart) + Cost(cGoal) > getLevelSet())
    {
      ...
      std::tie(cStart, cGoal)
          = HRS(startIndex, mid_index, sample, timeLeft - timeElapsedDouble);
      ...
      std::tie(c_dash_start, c_dash_goal)
          = HRS(mid_index + 1, endIndex, sample, timeLeft - timeElapsedDouble);
      cStart = combineCosts(
          getStartState(),
          sample,
          startIndex,
...
///
double GeometricHierarchicalRejectionSampler::calculateLeaf(
...
///
double GeometricHierarchicalRejectionSampler::combineCosts(
...
///
void GeometricHierarchicalRejectionSampler::sampleLeaf(
    Eigen::VectorXd& sample, const int dof)
{
...
/// @return Cost to go from x1 to x2
///
double DimtHierarchicalRejectionSampler::calculateLeaf(
    const Eigen::VectorXd& x1, const Eigen::VectorXd& x2, const int i)
{
  double cost, infeasible_min, infeasible_max;
  std::tie(cost, infeasible_min, infeasible_max)
      = dimt_->getMinTimeAndIntervals1Dof(
          x1[i * 2], x1[i * 2 + 1], x2[i * 2], x2[i * 2 + 1], i);
...
std::pair<std::size_t, double> max_in_range(
...
std::pair<bool, double> find_infeasible_intervals(
    ...
    const std::size_t end)
{
  for (std::size_t i = start; i <= end; i++)
  {
    if (max_val > std::get<0>(intervals[i])
        and max_val < std::get<1>(intervals[i]))
    {
...
/// @return Combination of the costs
///
double DimtHierarchicalRejectionSampler::combineCosts(
    const Eigen::VectorXd& x1,
    const Eigen::VectorXd& x2,
    const int i,
    const int m,
    ...
    const double c2) const
{
  ...
  while (is_invalid)
  {
    std::tie(is_invalid, max_val)
        = find_infeasible_intervals(infeasibleIntervals_, max_val, i, j);
  }
  return max_val;
}
...
/// @return A random vector in the space
///
void DimtHierarchicalRejectionSampler::sampleLeaf(
    Eigen::VectorXd& sample, const int dof)
{
