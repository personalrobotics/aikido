...
//
ompl::base::Cost ompl::base::MyOptimizationObjective::stateCost(
    const ompl::base::State* s) const
{
  if (sampler_ == nullptr or opt_ == nullptr)
  {
    throw std::runtime_error(
        "An informed sampler with optimization objective "
...
//
::ompl::base::Cost ompl::base::MyOptimizationObjective::motionCost(
...
ompl::base::MyOptimizationObjective::allocInformedStateSampler(
    const ProblemDefinitionPtr probDefn, unsigned int maxNumberCalls) const
{
  ...
  return sampler_;
  // return
  // ompl::base::OptimizationObjective::allocInformedStateSampler(probDefn,
  // maxNumberCalls);
}
...
{
  return Cost(
      dimt_->getMinTime(startState_, s) + dimt_->getMinTime(s, goalState_));
}
...
Cost DimtObjective::getCostIfSmallerThan(
