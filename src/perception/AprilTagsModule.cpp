...
//==============================================================================
AprilTagsModule::AprilTagsModule(
    ros::NodeHandle node,
    ...
    ros::Time timestamp)
{
  ...
  {
    dtwarn << "[AprilTagsModule::detectObjects] No markers on topic "
           << mMarkerTopic << std::endl;
    return false;
  }
            ...
            tag_name, skel_name, skel_resource, skel_offset))
    {
      ...
      try
      {
        mListener.waitForTransform(
            mReferenceFrameId, detection_frame, t, timeout);

        mListener.lookupTransform(
            mReferenceFrameId, detection_frame, t, transform);
      }
      catch (const tf::ExtrapolationException& ex)
      {
        dtwarn << "[AprilTagsModule::detectObjects] TF timestamp is "
      ...
      if (env_skeleton == nullptr)
      {
        ...
        if (!skel_to_update)
        {
          dtwarn << "[AprilTagsModule::detectObjects] Failed to load skeleton "
                    "for URI "
      ...
      {
        dtwarn << "[AprilTagsModule::detectObjects] Skeleton " << skel_name
               << " has 0 DOFs! \n";
        continue;
      }
      ...
      {
        dtwarn << "[AprilTagsModule::detectObjects] Could not cast the joint "
  ...
  if (!any_valid)
  {
    dtwarn << "[AprilTagsModule::detectObjects] No marker up-to-date with "
              "timestamp parameter\n";
    return false;
  }
