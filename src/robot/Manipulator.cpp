...
Manipulator::Manipulator(
  ...
  : Robot(name, robot, statespace, simulation, rngSeed, executor, retimer, collisionResolution)
  , mHand(std::move(hand))
{
  if (!mHand)
...
trajectory::TrajectoryPtr Manipulator::planToEndEffectorOffset(const statespace::dart::MetaSkeletonStateSpacePtr &space,
   ...
   double linearVelocity)
{
  ...
  auto trajectory = util::planToEndEffectorOffset(space, metaSkeleton, body, direction, collision, distance, linearVelocity);
...
Eigen::Vector3d Manipulator::getEndEffectorDirection(
      const dart::dynamics::BodyNodePtr &body)
{
  return body->getWorldTransform().linear().col(2).normalized();
...
{
  auto collision = getCollisionConstraint(space, collisionFree);
  auto trajectory = util::planToEndEffectorOffset(space, metaSkeleton, body, collision, distance, linearVelocity);
}
