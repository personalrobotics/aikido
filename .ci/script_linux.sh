...
if [ $CODECOV = ON ]; then ./scripts/internal-run.sh CTEST_OUTPUT_ON_FAILURE=true make -C build/aikido aikido_coverage; else ./scripts/internal-run.sh make -C build/aikido test; fi
