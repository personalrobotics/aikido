...
TEST_F(SmoothPostProcessorTests, useShortcutting)
{
  ...
  evaluate(
    mTrajectory.get(), mTrajectory->getStartTime(), startPositions);
  evaluate(
    smoothedTrajectory.get(), smoothedTrajectory->getStartTime(), statePositions);
  EXPECT_EIGEN_EQUAL(startPositions, statePositions, mTolerance);

  evaluate(
    mTrajectory.get(), mTrajectory->getEndTime(), goalPositions);
  evaluate(
    smoothedTrajectory.get(), smoothedTrajectory->getEndTime(), statePositions);
...
TEST_F(SmoothPostProcessorTests, useBlend)
{
  ...
  std::cout << "Original: " << mOriginalTrajectoryLength << std::endl;
  auto smoothedTrajectory
      = testSmoothPostProcessor.postprocess(*mTrajectory, mRng, testable);
