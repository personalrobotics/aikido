...
#ifdef USE_NLOPT
  static double inequalConstraint(
...
public:
  ...
  /// @return true if a sample is gotten false, if not
  ///
  virtual bool sampleUniform(
      State* statePtr, const Cost& minCost, const Cost& maxCost) override;
  ...
  ///
  virtual bool isInLevelSet(
      const Eigen::VectorXd& state, double& stateCost) const
  {
  ...
  ///
  virtual Eigen::VectorXd getInvJacobian(
  ...
  ///
  virtual Eigen::VectorXd newtonRaphson(
      const Eigen::VectorXd& start, double levelSet_);

  Eigen::VectorXd findSolutionInLevelSet(
      Eigen::VectorXd& init, double levelSet_);

  void setSingleSampleTimelimit(double timelimit)
  {
    timelimit_ = timelimit;
  }
  double getSingleSampleTimelimit()
  {
    return timelimit_;
  }
  void setBatchSampleTimelimit(double timelimit)
  {
    batchTimelimit_ = timelimit;
  }
  double getBatchSampleTimelimit()
  {
    return batchTimelimit_;
  }
